interface ClearDemoConfirmModalProps {
  open: boolean
  onConfirm: () => void
  onClose: () => void
  loading: boolean
...
export default function ClearDemoConfirmModal({ open, onConfirm, onClose, loading }: ClearDemoConfirmModalProps) {
  if (!open) return null
    ...
    >
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed mb-6">
          This will permanently remove all demo products, sales, expenses, credits, and inventory history generated for your business.
          <br /><br />
          Your account, subscription, and store information will NOT be deleted.
          <br /><br />
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700 active:bg-red-800 rounded-lg transition-colors shadow-sm disabled:opacity-50"
          >
            {loading ? (
              <>
